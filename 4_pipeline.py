...
"""
End-to-end RAG: FAISS retrieval -> cross-encoder rerank -> context window -> strict prompt -> Groq (Llama).
...
def run_rag_pipeline(query: str, retriever) -> dict[str, object]:
    """
    Manual flow: query -> FAISS -> rerank -> context -> prompt -> Groq -> answer.
    ...
    logger.info("Calling Groq model=%s", _GROQ_MODEL)
    completion = client.chat.completions.create(
        model=_GROQ_MODEL,
        messages=[{"role": "user", "content": prompt}],
    )
    choice = completion.choices[0].message
    answer = (getattr(choice, "content", None) or "").strip()
